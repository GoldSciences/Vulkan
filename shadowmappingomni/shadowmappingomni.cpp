...
public:
	...
	struct {
		...
		VkPipeline													cubeMap							= VK_NULL_HANDLE;
	}															pipelines;
	...
	void														prepareCubeMap					()									{
		...
		VkFormat														format								= VK_FORMAT_R32_SFLOAT;		// 32 bit float format for higher precision
		...
		VK_EVAL(vkCreateImage		(device, &imageCreateInfo, nullptr, &shadowCubeMap.image));

		vkGetImageMemoryRequirements		(device, shadowCubeMap.image, &memReqs);

		memAllocInfo.allocationSize									= memReqs.size;
		memAllocInfo.memoryTypeIndex								= vulkanDevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		VK_EVAL(vkAllocateMemory	(device, &memAllocInfo, nullptr, &shadowCubeMap.deviceMemory));
		VK_EVAL(vkBindImageMemory	(device, shadowCubeMap.image, shadowCubeMap.deviceMemory, 0));
		...
		vks::tools::setImageLayout(layoutCmd, shadowCubeMap.image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, subresourceRange);
		...
		VK_EVAL(vkCreateSampler		(device, &sampler, nullptr, &shadowCubeMap.sampler));
		...
		VK_EVAL(vkCreateImageView	(device, &view, nullptr, &shadowCubeMap.view));
	}
	...
	void														prepareOffscreenFramebuffer		()									{
		...
		VK_EVAL(vkCreateImage		(device, &imageCreateInfo, nullptr, &offscreenPass.color.image));
		...
		VK_EVAL(vkCreateImage		(device, &imageCreateInfo, nullptr, &offscreenPass.depth.image));
	...
	void														updateCubeFace					(uint32_t faceIndex)				{
		...
		case 1:
			...
			viewMatrix													= glm::rotate(viewMatrix, glm::radians(180.0f), glm::vec3(1.0f, 0.0f, 0.0f));
			break;	// NEGATIVE_X
		case 2: viewMatrix												= glm::rotate(viewMatrix, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f)); break;	// POSITIVE_Y
		case 3: viewMatrix												= glm::rotate(viewMatrix, glm::radians( 90.0f), glm::vec3(1.0f, 0.0f, 0.0f)); break;	// NEGATIVE_Y
		case 4: viewMatrix												= glm::rotate(viewMatrix, glm::radians(180.0f), glm::vec3(1.0f, 0.0f, 0.0f)); break;	// POSITIVE_Z
		case 5: viewMatrix												= glm::rotate(viewMatrix, glm::radians(180.0f), glm::vec3(0.0f, 0.0f, 1.0f)); break;	// NEGATIVE_Z
		}

		vkCmdBeginRenderPass		(offscreenPass.commandBuffer, &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);										// Render scene from cube face's point of view
		vkCmdPushConstants			(offscreenPass.commandBuffer, pipelineLayouts.offscreen, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(glm::mat4), &viewMatrix);	// Update shader push constant block. Contains current face view matrix
		...
		vks::tools::setImageLayout	(offscreenPass.commandBuffer, offscreenPass.color.image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
		...
		vkCmdCopyImage(offscreenPass.commandBuffer, offscreenPass.color.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, shadowCubeMap.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyRegion);		// Put image copy into command buffer
		vks::tools::setImageLayout(offscreenPass.commandBuffer, offscreenPass.color.image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);	// Transform framebuffer color attachment back 
	}
	...
	void														buildOffscreenCommandBuffer		()									{
		...
		vks::tools::setImageLayout	(offscreenPass.commandBuffer, shadowCubeMap.image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);

		for (uint32_t face = 0; face < 6; ++face)
			updateCubeFace(face);

		// Change image layout for all cubemap faces to shader read after they have been copied
		vks::tools::setImageLayout	(offscreenPass.commandBuffer, shadowCubeMap.image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, subresourceRange);
	...
	void														setupDescriptorSetLayout		()									{
		...
		std::vector<VkDescriptorSetLayoutBinding>						setLayoutBindings				=
			...
			,	vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 1)	// Binding 1 : Fragment shader image sampler (cube map)
			};
		...
		// Offscreen pipeline layout. Push constants for cube map face view matrices
		...
		VK_EVAL(vkCreatePipelineLayout		(device, &pPipelineLayoutCreateInfo, nullptr, &pipelineLayouts.offscreen));
	}
	...
	void														setupDescriptorSets				()									{
		...
		VkDescriptorImageInfo											texDescriptor					= vks::initializers::descriptorImageInfo(shadowCubeMap.sampler, shadowCubeMap.view, VK_IMAGE_LAYOUT_GENERAL);
		std::vector<VkWriteDescriptorSet>								sceneDescriptorSets				=
			...
			,	vks::initializers::writeDescriptorSet(descriptorSets.scene, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &texDescriptor)				// Binding 1 : Fragment shader shadow sampler
			};
		vkUpdateDescriptorSets(device, static_cast<uint32_t>(sceneDescriptorSets.size()), sceneDescriptorSets.data(), 0, NULL);

		// Offscreen
		VK_EVAL(vkAllocateDescriptorSets	(device, &allocInfo, &descriptorSets.offscreen));

		std::vector<VkWriteDescriptorSet>								offScreenWriteDescriptorSets	=
			{	vks::initializers::writeDescriptorSet(descriptorSets.offscreen,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBuffers.offscreen.descriptor),	// Binding 0 : Vertex shader uniform buffer
			};
	...
	void														prepareOffscreenRenderpass		()									{
		...
		VK_EVAL(vkCreateRenderPass(device, &renderPassCreateInfo, nullptr, &offscreenPass.renderPass));
	}
	...
	void														preparePipelines				()									{
		...
		VK_EVAL(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.scene));
		...
		VK_EVAL(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.cubeMap));
	...
	void														prepareUniformBuffers			()									{
		VK_EVAL(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.offscreen, sizeof(uboOffscreenVS	)));	// Offscreen vertex shader uniform buffer 
		VK_EVAL(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.scene	, sizeof(uboVSscene		)));	// Scene vertex shader uniform buffer
		// Map persistent
		VK_EVAL(uniformBuffers.offscreen	.map());
