...
public:
	...
	void														saveScreenshot				(const char *filename)				{
		...
		// Create the image
		VkImage															dstImage;
		VK_EVAL(vkCreateImage(device, &imgCreateInfo, nullptr, &dstImage));
		...
		VK_EVAL(vkAllocateMemory	(device, &memAllocInfo, nullptr, &dstImageMemory));
		VK_EVAL(vkBindImageMemory	(device, dstImage, dstImageMemory, 0));
	...
	virtual void												keyPressed					(uint32_t keyCode)					{
		switch (keyCode) {
		case KEY_F2				:
		case GAMEPAD_BUTTON_A	: saveScreenshot("screenshot.ppm"); break;
		}
	}

	virtual void												getOverlayText				(VulkanTextOverlay *textOverlay_)	{
#if defined(__ANDROID__)
		textOverlay_->addText("\"Button A\" to save screenshot", 5.0f, 85.0f, VulkanTextOverlay::alignLeft);
#else
		textOverlay_->addText("\"F2\" to save screenshot", 5.0f, 85.0f, VulkanTextOverlay::alignLeft);
#endif
	}
};
