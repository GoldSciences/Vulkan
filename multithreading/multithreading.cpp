...
public:
	...
	struct ThreadData {
		...
		std::vector<VkCommandBuffer>								commandBuffer;		// One command buffer per render object
		std::vector<ThreadPushConstantBlock>						pushConstBlock;		// One push constant block per render object
		std::vector<ObjectData>										objectData;			// Per object information (position, rotation, etc.)
	};
	...
	void														updateCommandBuffers							(VkFramebuffer frameBuffer)							{
		...
		inheritanceInfo.framebuffer									= frameBuffer;		// Secondary command buffer also use the currently active framebuffer
		std::vector<VkCommandBuffer>									commandBuffers;	// Contains the list of secondary command buffers to be executed
	...
	void														loadMeshes										()													{
		...
		models.skysphere.loadFromFile(getAssetPath() + "models/sphere.obj"				, vertexLayout, 1.00f, vulkanDevice, queue);
