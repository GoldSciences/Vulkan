...
public:
	...
	void														loadTexture					(std::string fileName, VkFormat format, bool forceLinearTiling)		{
		...
		// numLevels = 1 + floor(log2(max(w, h, d))). Calculated as log2(max(width, height, depth))c + 1 (see specs)
		...
		for (int32_t i = 1; i < (int32_t)texture.mipLevels; i++) {
			...
			vks::tools::setImageLayout	(blitCmd, texture.image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipSubRange, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT);				// Transiton current mip level to transfer dest
			vkCmdBlitImage				(blitCmd, texture.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, texture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageBlit, VK_FILTER_LINEAR);														// Blit from previous level
			vks::tools::setImageLayout	(blitCmd, texture.image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, mipSubRange, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);	// Transiton current mip level to transfer source for read in next iteration
		}

		// After the loop, all mip layers are in TRANSFER_SRC layout, so transition all to SHADER_READ
		subresourceRange.levelCount									= texture.mipLevels;
		vks::tools::setImageLayout(blitCmd, texture.image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, texture.imageLayout, subresourceRange);
	...
	void														setupVertexDescriptions		()																	{
		...
		vertices.attributeDescriptions[1]							= vks::initializers::vertexInputAttributeDescription(VERTEX_BUFFER_BIND_ID, 1, VK_FORMAT_R32G32_SFLOAT, 3 * sizeof(float));		// Location 1 : Texture coordinates
		vertices.attributeDescriptions[2]							= vks::initializers::vertexInputAttributeDescription(VERTEX_BUFFER_BIND_ID, 2, VK_FORMAT_R32G32B32_SFLOAT, 5 * sizeof(float));	// Location 1 : Vertex normal
	...
	void														setupDescriptorSet			()																	{
		...
		writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBufferVS.descriptor));
		// Binding 1: Sampled image
		VkDescriptorImageInfo											textureDescriptor			= vks::initializers::descriptorImageInfo( VK_NULL_HANDLE, texture.view, texture.imageLayout);
		writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1, &textureDescriptor));
	...
	virtual void												keyPressed					(uint32_t keyCode)													{
		...
		case KEY_F				:
		case GAMEPAD_BUTTON_A	:	toggleSampler();	break;
		}
	}
