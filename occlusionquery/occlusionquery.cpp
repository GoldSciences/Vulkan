...
public:
	...
	struct {
		...
		VkPipeline													occluder						= VK_NULL_HANDLE;
		VkPipeline													simple							= VK_NULL_HANDLE;	// Pipeline with basic shaders used for occlusion pass
	}															pipelines;

	struct {
		VkDescriptorSet												teapot							= VK_NULL_HANDLE;
	...
	void														getQueryResults					()										{
		...
		vkGetQueryPoolResults(device, queryPool, 0, 2, sizeof(passedSamples), passedSamples, sizeof(uint64_t),
	...
	void														buildCommandBuffers				()										{
		...
		for (size_t i = 0; i < drawCmdBuffers.size(); ++i) {
			...
			clearRect.rect.extent										= { screenSize.Width, screenSize.Height };
