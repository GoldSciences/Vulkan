...
public:
	...
	std::vector<BoneInfo>										boneInfo;									// Bone details
	uint32_t													numBones						= 0;		// Number of bones present
	aiMatrix4x4													globalInverseTransform;						// Root inverese transform matrix
	std::vector<VertexBoneData>									bones;										// Per-vertex bone info
	std::vector<aiMatrix4x4>									boneTransforms;								// Bone transformations
	float														animationSpeed					= 0.75f;	// Modifier for the animation 
	aiAnimation													* pAnimation					= nullptr;	// Currently active animation
...
public:
	...
	struct {
		VkPipeline													skinning						= VK_NULL_HANDLE;
		VkPipeline													texture							= VK_NULL_HANDLE;
	}															pipelines;
	...
	void														loadMesh						()																					{
		...
		VK_EVAL(vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, vertexBufferSize, &vertexStaging.buffer, &vertexStaging.memory, vertexBuffer.data()));	// Vertex data
		VK_EVAL(vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, indexBufferSize, &indexStaging.buffer, &indexStaging.memory, indexBuffer.data()));		// Index data

		// Create device local buffers
		VK_EVAL(vulkanDevice->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &skinnedMesh->vertexBuffer.vertices, vertexBufferSize));									// Vertex buffer
		VK_EVAL(vulkanDevice->createBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &skinnedMesh->vertexBuffer.indices, indexBufferSize));										// Index buffer
		...
		vkCmdCopyBuffer(copyCmd, vertexStaging.buffer, skinnedMesh->vertexBuffer.vertices.buffer, 1, &copyRegion);

		copyRegion.size												= indexBufferSize;
		vkCmdCopyBuffer(copyCmd, indexStaging.buffer, skinnedMesh->vertexBuffer.indices.buffer, 1, &copyRegion);
	...
	void														setupDescriptorSet				()																					{
		...
		std::vector<VkWriteDescriptorSet>								writeDescriptorSets				=
			{	vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBuffers.mesh.descriptor)		// Binding 0 : Vertex shader uniform buffer
		...
		writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(descriptorSets.floor, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBuffers.floor.descriptor));		// Binding 0 : Vertex shader uniform buffer
		writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(descriptorSets.floor, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &texDescriptor));				// Binding 1 : Color map 
		vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
	}
	...
	void														preparePipelines				()																					{
		...
		VK_EVAL(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.skinning));
		...
		VK_EVAL(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.texture));
	}

	// Prepare and initialize uniform buffer containing shader uniforms
	void														prepareUniformBuffers					()																			{
		// Mesh uniform buffer block
		vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.mesh, sizeof(uboVS));
		VK_EVAL(uniformBuffers.mesh.map());		// Map persistant

		// Floor uniform buffer block
		vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.floor, sizeof(uboFloor));
		VK_EVAL(uniformBuffers.floor.map());	// Map persistant
	...
	void														updateUniformBuffers					(bool viewChanged)															{
		if (viewChanged) {
			uboVS.projection										= glm::perspective(glm::radians(60.0f), (float)screenSize.Width / (float)screenSize.Height, 0.1f, 1024.0f);
	...
	virtual void												viewChanged								()																			{ vkDeviceWaitIdle(device); updateUniformBuffers(true); }
	void														changeAnimationSpeed					(float delta)																{ skinnedMesh->animationSpeed += delta; }
	virtual void												keyPressed								(uint32_t keyCode)															{
		switch (keyCode) {
		case KEY_KPADD			:
		case GAMEPAD_BUTTON_R1	:	changeAnimationSpeed(0.1f);		break;
		case KEY_KPSUB			:
		case GAMEPAD_BUTTON_L1	:	changeAnimationSpeed(-0.1f);	break;
		}
	}
