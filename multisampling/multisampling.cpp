...
public:
	...
	VkDescriptorSetLayout										descriptorSetLayout								= VK_NULL_HANDLE;
		...
		// Destroy MSAA target
		multisampleTarget.color.	destroy(device);
		multisampleTarget.depth.	destroy(device);
	...
	void														setupMultisampleTarget							()						{
		...
		info.extent.width											= screenSize.Width;
		info.extent.height											= screenSize.Height;
		info.extent.depth											= 1;
		info.mipLevels												= 1;
		info.arrayLayers											= 1;
	...
	virtual void												keyPressed										(uint32_t keyCode)		{
		switch (keyCode) {
		case KEY_S				:
		case GAMEPAD_BUTTON_A	:	toggleSampleShading();	break;
		}
	}
};
