...
public:
	...
	UBOVS 														uboVS											= {{},{}, glm::vec4(0.0, -2.0, 1.0, 0.0)};
	...
	struct {
		...
		camera.setPerspective	(60.0f, ((float)screenSize.Width / 3.0f) / (float)screenSize.Height, 0.1f, 512.0f);
	...
	void														buildCommandBuffers			()			{		 
		...
		for (size_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			// Left
			viewport.width												= (float)screenSize.Width / 3.0f;
			vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);
			vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.phong);
			vkCmdDrawIndexed(drawCmdBuffers[i], models.cube.indexCount, 1, 0, 0, 0);

			// Center
			viewport.x													= (float)screenSize.Width / 3.0f;
			vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);
			vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.toon);
			vkCmdDrawIndexed(drawCmdBuffers[i], models.cube.indexCount, 1, 0, 0, 0);

			// Right
			viewport.x													= (float)screenSize.Width / 3.0f + (float)screenSize.Width / 3.0f;
			vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);
	...
	void														setupVertexDescriptions		()			{
		...
		vertices.attributeDescriptions								= 
			...
			,	vks::initializers::vertexInputAttributeDescription(VERTEX_BUFFER_BIND_ID, 2, VK_FORMAT_R32G32_SFLOAT	, sizeof(float) * 6)	// Location 3 : Texture coordinates
			,	vks::initializers::vertexInputAttributeDescription(VERTEX_BUFFER_BIND_ID, 3, VK_FORMAT_R32G32B32_SFLOAT	, sizeof(float) * 8)	// Location 2 : Normal
			};
	...
	void														preparePipelines			()			{
		...
		VK_EVAL(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.textured));
	}
