...
public:
	...
	struct {
		struct Offscreen : public FrameBuffer {	
			vks::FrameBufferAttachmentWithFormat						position, normal, albedo, depth;
		}															offscreen;
		struct SSAO : public FrameBuffer {
			vks::FrameBufferAttachmentWithFormat						color;
		}															ssao, ssaoBlur;
	}															frameBuffers;
	...
	VkSemaphore													offscreenSemaphore									= VK_NULL_HANDLE;	// Semaphore used to synchronize between offscreen and final scene rendering
		...
		camera.setPerspective	(60.0f, (float)screenSize.Width / (float)screenSize.Height, 0.1f, 64.0f);
	}
	...
	void														createAttachment									(VkFormat format, VkImageUsageFlagBits usage, vks::FrameBufferAttachmentWithFormat* attachment, uint32_t width_, uint32_t height_)		{
	...
	void														prepareOffscreenFramebuffers					()									{
...
#else
		const uint32_t													ssaoWidth										= screenSize.Width;
		const uint32_t													ssaoHeight										= screenSize.Height;
#endif

		frameBuffers.offscreen	.setSize(screenSize.Width, screenSize.Height);
		frameBuffers.ssao		.setSize(ssaoWidth, ssaoHeight);
		frameBuffers.ssaoBlur	.setSize(screenSize.Width, screenSize.Height);
		...
		// G-Buffer 
		createAttachment(VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT	, &frameBuffers.offscreen.position	, screenSize.Width, screenSize.Height);	// Position + Depth
		createAttachment(VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT		, &frameBuffers.offscreen.normal	, screenSize.Width, screenSize.Height);	// Normals
		createAttachment(VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT		, &frameBuffers.offscreen.albedo	, screenSize.Width, screenSize.Height);	// Albedo (color)
		createAttachment(attDepthFormat, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT		, &frameBuffers.offscreen.depth		, screenSize.Width, screenSize.Height);	// Depth
		...
		// SSAO blur
		createAttachment(VK_FORMAT_R8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.ssaoBlur.color, screenSize.Width, screenSize.Height);					// Color
		...
		// G-Buffer creation
		{
			...
			VK_EVAL(vkCreateRenderPass(device, &renderPassInfo, nullptr, &frameBuffers.offscreen.renderPass));
			...
			VK_EVAL(vkCreateFramebuffer(device, &fbufCreateInfo, nullptr, &frameBuffers.offscreen.frameBuffer));
		}
		...
		// SSAO 
		{
			...
			renderPassInfo.dependencyCount								= 2;
			renderPassInfo.pDependencies								= dependencies.data();
			VK_EVAL(vkCreateRenderPass(device, &renderPassInfo, nullptr, &frameBuffers.ssao.renderPass));
			...
			fbufCreateInfo.layers										= 1;
			VK_EVAL(vkCreateFramebuffer(device, &fbufCreateInfo, nullptr, &frameBuffers.ssao.frameBuffer));
		}
		...
		// SSAO Blur 
		{
			...
			renderPassInfo.dependencyCount								= 2;
			renderPassInfo.pDependencies								= dependencies.data();
			VK_EVAL(vkCreateRenderPass(device, &renderPassInfo, nullptr, &frameBuffers.ssaoBlur.renderPass));
			...
			fbufCreateInfo.layers										= 1;
			VK_EVAL(vkCreateFramebuffer(device, &fbufCreateInfo, nullptr, &frameBuffers.ssaoBlur.frameBuffer));
		}
		...
		VK_EVAL(vkCreateSampler(device, &sampler, nullptr, &colorSampler));
	}
	...
	void														buildDeferredCommandBuffer						()									{
		...
		VK_EVAL(vkBeginCommandBuffer	(offScreenCmdBuffer, &cmdBufInfo));
		...
		VK_EVAL(vkEndCommandBuffer(offScreenCmdBuffer));
	}
	...
	void														setupLayoutsAndDescriptors					()									{
		...
		VK_EVAL(vkCreateDescriptorSetLayout	(device, &setLayoutCreateInfo, nullptr, &descriptorSetLayouts.gBuffer));
		pipelineLayoutCreateInfo.pSetLayouts						= &descriptorSetLayouts.gBuffer;
		VK_EVAL(vkCreatePipelineLayout		(device, &pipelineLayoutCreateInfo, nullptr, &pipelineLayouts.gBuffer));
		descriptorAllocInfo.pSetLayouts								= &descriptorSetLayouts.gBuffer;
		VK_EVAL(vkAllocateDescriptorSets	(device, &descriptorAllocInfo, &descriptorSets.floor));
		writeDescriptorSets											= 
		...
		setLayoutCreateInfo											= vks::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()));
		VK_EVAL(vkCreateDescriptorSetLayout	(device, &setLayoutCreateInfo, nullptr, &descriptorSetLayouts.ssao));
		pipelineLayoutCreateInfo.pSetLayouts						= &descriptorSetLayouts.ssao;
		VK_EVAL(vkCreatePipelineLayout		(device, &pipelineLayoutCreateInfo, nullptr, &pipelineLayouts.ssao));
		descriptorAllocInfo.pSetLayouts								= &descriptorSetLayouts.ssao;
		VK_EVAL(vkAllocateDescriptorSets	(device, &descriptorAllocInfo, &descriptorSets.ssao));
		imageDescriptors											= 
		...
		setLayoutCreateInfo											= vks::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()));
		VK_EVAL(vkCreateDescriptorSetLayout	(device, &setLayoutCreateInfo, nullptr, &descriptorSetLayouts.ssaoBlur));
		pipelineLayoutCreateInfo.pSetLayouts						= &descriptorSetLayouts.ssaoBlur;
		VK_EVAL(vkCreatePipelineLayout		(device, &pipelineLayoutCreateInfo, nullptr, &pipelineLayouts.ssaoBlur));
		descriptorAllocInfo.pSetLayouts								= &descriptorSetLayouts.ssaoBlur;
		VK_EVAL(vkAllocateDescriptorSets	(device, &descriptorAllocInfo, &descriptorSets.ssaoBlur));
		// todo
		imageDescriptors											= 
		...
		setLayoutCreateInfo											= vks::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()));
		VK_EVAL(vkCreateDescriptorSetLayout	(device, &setLayoutCreateInfo, nullptr, &descriptorSetLayouts.composition));
		...
		VK_EVAL(vkAllocateDescriptorSets	(device, &descriptorAllocInfo, &descriptorSets.composition));
		imageDescriptors											= 
			{	vks::initializers::descriptorImageInfo(colorSampler, frameBuffers.offscreen	.position	.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
			,	vks::initializers::descriptorImageInfo(colorSampler, frameBuffers.offscreen	.normal		.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
	...
	void														preparePipelines							()									{
		...
		shaderStages[1]												= loadShader(getAssetPath() + "shaders/ssao/ssao.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
		{
			...
			VK_EVAL(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.ssao));
		}
		...
		VK_EVAL(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.ssaoBlur));
	...
	void														prepareUniformBuffers						()									{
		vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.sceneMatrices, sizeof(uboSceneMatrices	));	// Scene matrices
		vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.ssaoParams	, sizeof(uboSSAOParams		));	// SSAO parameters 
		...
		// Upload as UBO
		vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.ssaoKernel, ssaoKernel.size() * sizeof(glm::vec4), ssaoKernel.data());
	...
	void														updateUniformBufferMatrices					()									{
		...
		VK_EVAL(uniformBuffers.sceneMatrices.map());
	...
	void														updateUniformBufferSSAOParams				()									{
		...
		VK_EVAL(uniformBuffers.ssaoParams.map());
	...
	void														toggleSSAO									()									{ uboSSAOParams.ssao		= !uboSSAOParams.ssao;		updateUniformBufferSSAOParams();	}
	void														toggleSSAOBlur								()									{ uboSSAOParams.ssaoBlur	= !uboSSAOParams.ssaoBlur;	updateUniformBufferSSAOParams();	}
	void														toggleSSAOOnly								()									{ uboSSAOParams.ssaoOnly	= !uboSSAOParams.ssaoOnly;	updateUniformBufferSSAOParams();	}
	virtual void												viewChanged									()									{ updateUniformBufferMatrices();						updateUniformBufferSSAOParams();	}
	virtual void												render										()									{ if (!prepared) return; draw();															}
	virtual void												keyPressed									(uint32_t keyCode)					{
		switch (keyCode) {
		case KEY_F2				:
		case GAMEPAD_BUTTON_A	:	toggleSSAO		(); break;
		case KEY_F3				:
		case GAMEPAD_BUTTON_X	:	toggleSSAOBlur	(); break;
		case KEY_F4				:
		case GAMEPAD_BUTTON_Y	:	toggleSSAOOnly	(); break;
		}
	}
