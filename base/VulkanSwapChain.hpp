...
public:
	...
	VkSwapchainKHR										swapChain									= VK_NULL_HANDLE;		// Handle to the current swap chain, required for recreation
	uint32_t											imageCount;
	...
	*/
	...
	void												create										(uint32_t *width_, uint32_t *height_, bool vsync = false)								{
		...
		preTransform										= (surfCaps.supportedTransforms & VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR) ? VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR : surfCaps.currentTransform;	// We prefer a non-rotated transform
	...
	//  
	// Acquires the next image in the swap chain. Returns VkResult of the image acquisition.
	// 
	...
	VkResult											acquireNextImage							(VkSemaphore presentCompleteSemaphore, uint32_t *imageIndex)						{
		// By setting timeout to UINT64_MAX we will always wait until the next image has been acquired or an actual error is thrown. With that we don't have to handle VK_NOT_READY
		return fpAcquireNextImageKHR(device, swapChain, UINT64_MAX, presentCompleteSemaphore, (VkFence)nullptr, imageIndex);
	}

	// Queue an image for presentation. Returns VkResult of the queue presentation.
	// 
	...
	void												createDirect2DisplaySurface					(uint32_t width, uint32_t height)													{
		...
		VkDisplayPlaneAlphaFlagBitsKHR							alphaMode;
			 ...
			 if (planeCap.supportedAlpha & VK_DISPLAY_PLANE_ALPHA_PER_PIXEL_PREMULTIPLIED_BIT_KHR)	alphaMode		= VK_DISPLAY_PLANE_ALPHA_PER_PIXEL_PREMULTIPLIED_BIT_KHR;
		else if (planeCap.supportedAlpha & VK_DISPLAY_PLANE_ALPHA_PER_PIXEL_BIT_KHR)				alphaMode		= VK_DISPLAY_PLANE_ALPHA_PER_PIXEL_BIT_KHR;
		else																						alphaMode		= VK_DISPLAY_PLANE_ALPHA_GLOBAL_BIT_KHR;
