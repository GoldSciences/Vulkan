...
namespace vks
{
	...
	struct Texture2D : public Texture {
		...
		)
		{
			...
			else
			{
				...
				VK_CHECK_RESULT(vkCreateImage(device->logicalDevice, &imageCreateInfo, nullptr, &mappableImage));		// Load mip map level 0 to linear tiling image

				vkGetImageMemoryRequirements(device->logicalDevice, mappableImage, &memReqs);							// Get memory requirements for this image, like size and alignment.
				memAllocInfo.allocationSize							= memReqs.size;											// Set memory allocation size to required memory size

				// Get memory type that can be mapped to host memory
				memAllocInfo.memoryTypeIndex						= device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

				VK_CHECK_RESULT(vkAllocateMemory(device->logicalDevice, &memAllocInfo, nullptr, &mappableMemory));		// Allocate host memory
				VK_CHECK_RESULT(vkBindImageMemory(device->logicalDevice, mappableImage, mappableMemory, 0));			// Bind allocated image for use
				...
				vkGetImageSubresourceLayout(device->logicalDevice, mappableImage, &subRes, &subResLayout);				// Get sub resources layout. Includes row pitch, size offsets, etc.
				VK_CHECK_RESULT(vkMapMemory(device->logicalDevice, mappableMemory, 0, memReqs.size, 0, &data));			// Map image memory
		...
		)
		{
			...
			VkCommandBuffer											copyCmd							= device->createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);	// Use a separate command buffer for texture loading
	...
	struct Texture2DArray : public Texture {
		...
		)
		{
			...
			this->imageLayout									= imageLayout;
			vks::tools::setImageLayout(copyCmd, image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, imageLayout, subresourceRange);
	...
	public:
		...
		)
		{
			...
			VK_CHECK_RESULT(vkCreateBuffer(device->logicalDevice, &bufferCreateInfo, nullptr, &stagingBuffer));
			vkGetBufferMemoryRequirements(device->logicalDevice, stagingBuffer, &memReqs);																										// Get memory requirements for the staging buffer (alignment, memory type bits)

			memAllocInfo.allocationSize							= memReqs.size;
			memAllocInfo.memoryTypeIndex						= device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);	// Get memory type index for a host visible buffer
			...
			this->imageLayout = imageLayout;
			vks::tools::setImageLayout(copyCmd, image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, imageLayout, subresourceRange);									// Change texture image layout to shader read after all faces have been copied.
