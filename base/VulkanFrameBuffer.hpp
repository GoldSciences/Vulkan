...
namespace vks
{
	...
	// Encapsulates a complete Vulkan framebuffer with an arbitrary number and combination of attachments
	struct Framebuffer
	{
	private:
	...
	public:
		...
		std::vector<vks::FramebufferAttachment>	attachments;
												...
												Framebuffer			(vks::VulkanDevice *in_vulkanDevice)										: vulkanDevice(in_vulkanDevice)	{ assert(vulkanDevice); }
												~Framebuffer		()																			{
		...
		// Add a new attachment described by createinfo to the framebuffer's attachment list. Returns index of the new attachment. 
		// createinfo: Structure that specifices the framebuffer to be constructed
		...
		// Creates a default sampler for sampling from any of the framebuffer attachments. Applications are free to create their own samplers for different use cases.
		// 
		// magFilter Magnification filter for lookups
		// minFilter Minification filter for lookups
		// adressMode Adressing mode for the U,V and W coordinates
		// 
		// Returns VkResult for the sampler creation
		...
		// Creates a default render pass setup with one sub pass. Returns VK_SUCCESS if all resources have been created successfully
