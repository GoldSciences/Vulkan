...
{	
	struct VertexBuffer	{
		...
		void													Destroy					(VkDevice device_)												{
			if(VK_NULL_HANDLE != Buffer)	{	vkDestroyBuffer	(device_, Buffer, nullptr);	Buffer	= VK_NULL_HANDLE;	}
			if(VK_NULL_HANDLE != Memory)	{	vkFreeMemory	(device_, Memory, nullptr);	Memory	= VK_NULL_HANDLE;	}
		}
	};
