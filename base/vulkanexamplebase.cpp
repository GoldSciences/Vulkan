...
void										VulkanExampleBase::renderLoop						()																												{
	...
	while (!quit) {
		...
		if (fpsTimer > 1000.0f) {
			...
			lastFPS														ss= frameCounter;
			updateTextOverlay();										ss
			fpsTimer													ss= 0.0f;
			frameCounter												ss= 0;
		}
	}
...
void										VulkanExampleBase::seatCapabilities					(wl_seat *seat, uint32_t caps)																														{
	if ((caps & WL_SEAT_CAPABILITY_POINTER) && !pointer) {
		...
		wl_pointer_add_listener(pointer, &pointer_listener, this);s
	}
...
static void									PingCb												(void *data, struct wl_shell_surface *shell_surface, uint32_t serial)											{ wl_shell_surface_pong(shell_surface, serial); }
static void									ConfigureCb											(void *data, struct wl_shell_surface *shell_surface, uint32_t edges, int32_t width, int32_t height)				{}
