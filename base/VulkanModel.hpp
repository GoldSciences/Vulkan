...
namespace vks
{
	...
	struct Model {
		...
		// Stores vertex and index base and counts for each part of a model */
		struct ModelPart {
			uint32_t						vertexBase;
		...
		// Release all Vulkan resources of this model
		void							destroy					()																							{
			assert(device);
		...
		// Loads a 3D model from a file into Vulkan buffers
		// 
		// device			: Pointer to the Vulkan device used to generated the vertex and index buffers on
		// filename			: File to load (must be a model format supported by ASSIMP)
		// layout			: Vertex layout components (position, normals, tangents, etc.)
		// scale			: Load time scene scale
		// copyQueue		: Queue used for the memory staging copy commands (must support transfer)
		// (Optional) flags	: ASSIMP model loading flags
