...
public:
	...
	void														buildCommandBuffers								()									{
		...
		for (size_t i = 0; i < drawCmdBuffers.size(); ++i) {
			...
			vkCmdBindVertexBuffers	(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1, &models.rock.vertices.buffer, offsets);	// Binding point 0 : Mesh vertex buffer	
			...
			vkCmdDrawIndexed		(drawCmdBuffers[i], models.rock.indexCount, INSTANCE_COUNT, 0, 0, 0);					// Render instances
	...
	void														preparePipelines								()									{
		...
		bindingDescriptions											= 
			{	vks::initializers::vertexInputBindingDescription(VERTEX_BUFFER_BIND_ID, vertexLayout.stride(), VK_VERTEX_INPUT_RATE_VERTEX)		// Binding point 0: Mesh vertex layout description at per-vertex rate
			,	vks::initializers::vertexInputBindingDescription(INSTANCE_BUFFER_BIND_ID, sizeof(InstanceData), VK_VERTEX_INPUT_RATE_INSTANCE)	// Binding point 1: Instanced data at per-instance rate
			};
		...
		attributeDescriptions										= 
				// Per-vertex attributees. These are advanced for each vertex fetched by the vertex shader
			...
			,	vks::initializers::vertexInputAttributeDescription(VERTEX_BUFFER_BIND_ID, 3, VK_FORMAT_R32G32B32_SFLOAT, sizeof(float) * 8)		// Location 3: Color
				// Per-Instance attributes. These are fetched for each instance rendered
