...
public:
	...
	void														prepareTextureTarget					(vks::Texture *tex, uint32_t width_, uint32_t height_, VkFormat format)		{
		...
		vkGetPhysicalDeviceFormatProperties(physicalDevice, format, &formatProperties);				// Get device properties for the requested texture format
	...
	void														loadAssets								()																			{
		textureColorMap.loadFromFile(getAssetPath() + "textures/het_kanonschot_rgba8.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
	}
	...
	void														setupDescriptorPool						()																			{
		std::vector<VkDescriptorPoolSize>								poolSizes								=
			...
			,	vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1)			// Compute pipeline uses a sampled image for reading
			,	vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 2)			// Compute pipelines uses a storage image for image reads and writes
			};
	...
	void														setupDescriptorSet						()																			{
		...
		VK_EVAL(vkAllocateDescriptorSets(device, &allocInfo, &graphics.descriptorSetPostCompute));

		std::vector<VkWriteDescriptorSet>								writeDescriptorSets						=
			{	vks::initializers::writeDescriptorSet(graphics.descriptorSetPostCompute, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER			, 0, &uniformBufferVS		.descriptor)	// Binding 0 : Vertex shader uniform buffer
			,	vks::initializers::writeDescriptorSet(graphics.descriptorSetPostCompute, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER	, 1, &textureComputeTarget	.descriptor)	// Binding 1 : Fragment shader texture sampler
			};
		...
		VK_EVAL(vkAllocateDescriptorSets(device, &allocInfo, &graphics.descriptorSetPreCompute));

		std::vector<VkWriteDescriptorSet>								baseImageWriteDescriptorSets			=
			{	vks::initializers::writeDescriptorSet(graphics.descriptorSetPreCompute, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER			, 0, &uniformBufferVS.descriptor)	// Binding 0 : Vertex shader uniform buffer
			,	vks::initializers::writeDescriptorSet(graphics.descriptorSetPreCompute, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER	, 1, &textureColorMap.descriptor)	// Binding 1 : Fragment shader texture sampler
			};
	...
	void														prepareCompute							()																			{
		...
		for (const std::string& shaderName : shaderNames) {
			...
			VkPipeline														pipeline;
			VK_EVAL(vkCreateComputePipelines(device, pipelineCache, 1, &computePipelineCreateInfo, nullptr, &pipeline));
	...
	void														prepareUniformBuffers					()																			{
		...
		VK_EVAL(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBufferVS, sizeof(uboVS)));	// Vertex shader uniform buffer block
		VK_EVAL(uniformBufferVS.map());		// Map persistent
