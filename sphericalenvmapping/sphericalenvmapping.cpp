// Vulkan Example - Spherical Environment Mapping, using different mat caps. Use +/-/space toggle through different material captures.
// 
// Based on https://www.clicktorelease.com/blog/creating-spherical-environment-mapping-shader
// 
// Copyright (C) 2016 by Sascha Willems - www.saschawillems.de. This code is licensed under the MIT license (MIT) (http://opensource.org/licenses/MIT)
...
public:
	...
	void														setupDescriptorSet						()										{
		...
		std::vector<VkWriteDescriptorSet>								writeDescriptorSets						=
			...
			,	vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &textures.matCapArray.descriptor)	// Binding 1 : Fragment shader image sampler
			};
	...
	void														updateUniformBuffers					()										{
		uboVS.projection											= glm::perspective(glm::radians(45.0f), (float)screenSize.Width / (float)screenSize.Height, 0.1f, 256.0f);
	...
	virtual void												keyPressed								(uint32_t keyCode)						{
		...
		case KEY_KPADD			:
		case KEY_SPACE			:
		case GAMEPAD_BUTTON_A	: changeMatCapIndex(1);		break;
		case KEY_KPSUB			:
		case GAMEPAD_BUTTON_X	: changeMatCapIndex(-1);	break;
		}
	}
	...
	void														changeMatCapIndex						(uint32_t delta)						{
		...
		if (uboVS.texIndex < 0)											uboVS.texIndex	= textures.matCapArray.layerCount-1;
		if (uboVS.texIndex >= (int32_t)textures.matCapArray.layerCount)	uboVS.texIndex	= 0;
		updateUniformBuffers();
	}
