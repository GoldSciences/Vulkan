...
private:
	...
	T												fade						(T t)						const	{ return t * t * t * (t * (t * (T)6 - (T)15) + (T)10);	}
	T												lerp						(T t, T a, T b)				const	{ return a + t * (b - a);								}
...
#define FRACTAL
		...
		vks::tools::setImageLayout(copyCmd, texture.image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, texture.imageLayout, subresourceRange);
	...
	void preparePipelines()
	{
		...
		std::array<VkPipelineShaderStageCreateInfo,2>			shaderStages;			// Load shaders
