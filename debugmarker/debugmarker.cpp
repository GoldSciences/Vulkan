...
namespace DebugMarker
{
	...
	void														setup									(VkDevice device, VkPhysicalDevice physicalDevice)							{
		...
		if (extensionPresent) {
			...
			active														= (vkDebugMarkerSetObjectName != VK_NULL_HANDLE);	// Set flag if at least one function pointer is present
		}
		else {
...
struct Scene {
	...
	inline void													loadFromFile							(std::string filename, vks::VulkanDevice* vulkanDevice, VkQueue queue)		{ model.loadFromFile(filename, vertexLayout, 1.0f, vulkanDevice, queue); }
...
public:
	...
	void														buildCommandBuffers						()									{
		...
		for (size_t i = 0; i < drawCmdBuffers.size(); ++i) {
			...
			// Solid rendering.
			DebugMarker::beginRegion	(drawCmdBuffers[i], "Toon shading draw", glm::vec4(0.78f, 0.74f, 0.9f, 1.0f));	// Start a new debug marker region
			...
			if (glow) {
				...
				vkCmdDraw					(drawCmdBuffers[i], 4, 1, 0, 0);	// Full screen quad is generated by the vertex shaders, so we reuse four vertices (for four invocations) from current vertex buffer
	...
	void														preparePipelines						()									{
		...
		VkVertexInputBindingDescription									vertexInputBinding						= vks::initializers::vertexInputBindingDescription(VERTEX_BUFFER_BIND_ID, vertexLayout.stride(), VK_VERTEX_INPUT_RATE_VERTEX);	// Binding description
