...
private:
	...
	void														loadMaterials					()													{
		...
		for (size_t i = 0; i < materials.size(); i++) {
			...
			// Properties
			aiColor4D														color;
			aScene->mMaterials[i]->Get(AI_MATKEY_COLOR_AMBIENT	, color);	materials[i].properties.ambient		= glm::make_vec4(&color.r) + glm::vec4(0.1f);
			aScene->mMaterials[i]->Get(AI_MATKEY_COLOR_DIFFUSE	, color);	materials[i].properties.diffuse		= glm::make_vec4(&color.r);
			aScene->mMaterials[i]->Get(AI_MATKEY_COLOR_SPECULAR	, color);	materials[i].properties.specular	= glm::make_vec4(&color.r);
		...
		VkPushConstantRange												pushConstantRange				= vks::initializers::pushConstantRange(VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(SceneMaterialProperites), 0);
		...
		for (size_t i = 0; i < materials.size(); i++) {
			...
			writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(materials[i].descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &materials[i].diffuse.descriptor));
...
public:
	...
	VkPipelineLayout											pipelineLayout;					// Shared pipeline layout
	...
	// Default constructor
																Scene							(vks::VulkanDevice *vulkanDevice_, VkQueue queue)	{
		this->vulkanDevice											= vulkanDevice_;
		this->queue													= queue;
		...
		uniformBuffer.device										= vulkanDevice_->logicalDevice;
	}
	...
	void														render							(VkCommandBuffer cmdBuffer, bool wireframe)			{
		...
		for (size_t i = 0; i < meshes.size(); i++) {
			...
			descriptorSets[0]											= descriptorSetScene;					// Set 0: Scene descriptor set containing global matrices
			descriptorSets[1]											= meshes[i].material->descriptorSet;	// Set 1: Per-Material descriptor set containing bound images
			...
			vkCmdPushConstants		(cmdBuffer, pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(SceneMaterialProperites), &meshes[i].material->properties);		// Pass material properies via push constants
			vkCmdDrawIndexed		(cmdBuffer, meshes[i].indexCount, 1, 0, meshes[i].indexBase, 0);																	// Render from the global scene vertex buffer using the mesh index offset
		}
	}
};
...
public:
	...
	void														preparePipelines				()													{
		...
		VkGraphicsPipelineCreateInfo									pipelineCreateInfo				= vks::initializers::pipelineCreateInfo(scene->pipelineLayout, renderPass, 0);
