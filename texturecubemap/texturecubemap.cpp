...
public:
	...
	void														loadCubemap								(std::string filename, VkFormat format, bool forceLinearTiling)	{
		...
		cubeMap.mipLevels											= static_cast<uint32_t>(texCube.levels());
		...
		for (uint32_t face = 0; face < 6; face++)
			for (uint32_t level = 0; level < cubeMap.mipLevels; level++) {
				...
				offset														+= static_cast<uint32_t>(texCube[face][level].size());	// Increase offset into staging buffer for next level / face
			}
	...
	void														setupVertexDescriptions					()																{
		...
		vertices.attributeDescriptions[2]							= vks::initializers::vertexInputAttributeDescription(VERTEX_BUFFER_BIND_ID, 2, VK_FORMAT_R32G32_SFLOAT	, sizeof(float) * 5);		// Location 2 : Texture coordinates
	...
	void														setupDescriptorSets						()																{
		...
		std::vector<VkWriteDescriptorSet>								writeDescriptorSets						=
			{	vks::initializers::writeDescriptorSet(descriptorSets.object, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER			, 0, &uniformBuffers.object.descriptor)	// Binding 0 : Vertex shader uniform buffer
			,	vks::initializers::writeDescriptorSet(descriptorSets.object, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER	, 1, &textureDescriptor)				// Binding 1 : Fragment shader cubemap sampler
			};
		...
		writeDescriptorSets											=
			...
			,	vks::initializers::writeDescriptorSet(descriptorSets.skybox, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER	, 1,&textureDescriptor)					// Binding 1 : Fragment shader cubemap sampler
			};
		vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
	}
	...
	void														prepareUniformBuffers					()																{
		VK_EVAL(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.object, sizeof(uboVS)));		// Objact vertex shader uniform buffer
		VK_EVAL(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.skybox, sizeof(uboVS)));		// Skybox vertex shader uniform buffer

		// Map persistent
		VK_EVAL(uniformBuffers.object.map());
...
#if defined(__ANDROID__)
		textOverlay_->addText("Press \"Button A\" to toggle skybox", 5.0f, 85.0f, VulkanTextOverlay::alignLeft);
		textOverlay_->addText("Press \"Button X\" to toggle object", 5.0f, 100.0f, VulkanTextOverlay::alignLeft);
		textOverlay_->addText("LOD bias: " + ss.str() + " (Buttons L1/R1 to change)", 5.0f, 115.0f, VulkanTextOverlay::alignLeft);
#else
		textOverlay_->addText("Press \"s\" to toggle skybox", 5.0f, 85.0f, VulkanTextOverlay::alignLeft);
		textOverlay_->addText("Press \"space\" to toggle object", 5.0f, 100.0f, VulkanTextOverlay::alignLeft);
		textOverlay_->addText("LOD bias: " + ss.str() + " (numpad +/- to change)", 5.0f, 115.0f, VulkanTextOverlay::alignLeft);
#endif
	}
};
